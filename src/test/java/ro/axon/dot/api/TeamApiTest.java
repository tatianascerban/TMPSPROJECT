...
class TeamApiTest {
  ...
  @Test
  void getTeamListWhenStatusIsProvided_success() throws Exception {
    ...
    when(teamService.getTeamsDetails(any()))
        .thenReturn(teamDetailsList);
    mockMvc.perform(get("/api/v1/teams?status=ACTIVE")
            .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.items", hasSize(2)));
    verify(teamService).getTeamsDetails(Optional.of(TeamStatus.ACTIVE));
  }
  ...
  void getTeamList_success() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/teams")
        ...
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.items", hasSize(2)));
    verify(teamService).getTeamsDetails(Optional.empty());
  }
}